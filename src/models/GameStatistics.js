...
class GameStatistics {
   ...
   * @param {number} statsData.total_wins - Total number of games won
   ...
   * @param {number} statsData.current_streak - Current winning streak
   * @param {number} statsData.best_streak - Best winning streak achieved
  ...
  constructor(statsData) {
    ...
    this.current_streak = statsData.current_streak || 0;
    this.best_streak = statsData.best_streak || 0;
   ...
   * @returns {void}
   */
  updateWithGameSession(gameSession, isVictory = false) {
    // Increment total games
    this.total_games += 1;

    // Update wins if game was won
    if (isVictory) {
      this.total_wins += 1;
      ...
      // Reset current streak if game was lost
      this.current_streak = 0;
    }

    // Update score statistics
    this.total_score += gameSession.score;
    if (gameSession.score > this.best_score) {
      this.best_score = gameSession.score;
    }

    // Update lines cleared statistics
    this.total_lines_cleared += gameSession.lines_cleared;
    ...
    this.total_figures_placed += gameSession.figures_placed;

    // Update duration statistics
    this.total_duration += gameSession.duration;
    ...
    // Update moves count
    this.total_moves += gameSession.moves_count;
    ...
    this.updateRating(gameSession, isVictory);
   ...
   * @returns {void}
   */
  updateRating(gameSession, isVictory) {
    // Base rating change
    let ratingChange = 0;
  ...
  static aggregateStats(statsArray) {
    ...
    for (const stats of statsArray) {
      ...
      aggregated.total_wins += stats.total_wins;
      aggregated.total_score += stats.total_score;
      aggregated.total_lines_cleared += stats.total_lines_cleared;
      aggregated.total_figures_placed += stats.total_figures_placed;
      aggregated.total_duration += stats.total_duration;
      aggregated.total_moves += stats.total_moves;
      ...
      if (stats.best_streak > aggregated.best_streak) {
        aggregated.best_streak = stats.best_streak;
      }
   ...
   * @param {number} thresholds.minWinRate - Minimum win rate percentage
  ...
  meetsThresholds(thresholds) {
    ...
    if (thresholds.minWinRate && this.calculateWinRate() < thresholds.minWinRate) {
      return false;
    }
